...
STAGE="${HQC256_STAGE:-0}"
BENCH_ITERS="${HQC256_BENCH_ITERS:-100}"
...
OUT="$PROJECT_DIR/build/hqc256_decode_stage_bench_hexagon_stage${STAGE}"
mkdir -p "$(dirname "$OUT")"

echo "=== Compiling HQC-256 stage benchmark for Hexagon HVX intrinsic, stage=$STAGE, iters=$BENCH_ITERS, gf_lut=$GF_LUT_MUL, gf_hwstyle=$GF_HWSTYLE_MUL, hvx_rs_syndrome=$HVX_RS_SYNDROME ==="
"$CLANG" -O2 -mv75 \

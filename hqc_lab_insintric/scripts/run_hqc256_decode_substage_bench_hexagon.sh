...
SUBSTAGE="${HQC256_SUBSTAGE:-4}"
BENCH_ITERS="${HQC256_BENCH_ITERS:-10}"
...
OUT="$PROJECT_DIR/build/hqc256_decode_substage_bench_hexagon_stage${SUBSTAGE}"
mkdir -p "$(dirname "$OUT")"

echo "=== Compiling HQC-256 substage benchmark for Hexagon HVX intrinsic, substage=$SUBSTAGE, iters=$BENCH_ITERS, gf_lut=$GF_LUT_MUL, gf_hwstyle=$GF_HWSTYLE_MUL, hvx_rs_syndrome=$HVX_RS_SYNDROME ==="
"$CLANG" -O2 -mv75 \
    ...
    "${HVX_RS_SYNDROME_FLAGS[@]}" \
    -DARCHV=75 \
    -DHQC256_SUBSTAGE="$SUBSTAGE" \
    -DHQC256_BENCH_ITERS="$BENCH_ITERS" \
    ...
    -Wl,--gc-sections \
    -o "$OUT" \
    "$PROJECT_DIR/demos/hqc256_decode_substage_bench.c" \

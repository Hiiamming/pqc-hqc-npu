...
#define HQC256_BENCH_ITERS 10
#endif

#ifndef HQC256_SUBSTAGE
#define HQC256_SUBSTAGE 0
#endif
...
int main(void) {
    uint64_t codeword_words[HQC256_FIXTURE_COUNT][VEC_N1N2_SIZE_64] __attribute__((aligned(128))) = {{0}};
    uint64_t rs_words[HQC256_FIXTURE_COUNT][VEC_N1_SIZE_64] __attribute__((aligned(128))) = {{0}};
    uint64_t message_words[MSG_WORDS] = {0};

    rm_expanded_cdw rm_expanded_ref[HQC256_FIXTURE_COUNT][PARAM_N1] __attribute__((aligned(128)));
    rm_expanded_cdw rm_transform_ref[HQC256_FIXTURE_COUNT][PARAM_N1] __attribute__((aligned(128)));

    uint8_t rs_cdw_ref[HQC256_FIXTURE_COUNT][PARAM_N1] = {{0}};
    uint16_t rs_syndromes_ref[HQC256_FIXTURE_COUNT][2 * PARAM_DELTA] = {{0}};
    uint16_t rs_sigma_ref[HQC256_FIXTURE_COUNT][1 << PARAM_FFT] = {{0}};
    uint8_t rs_error_ref[HQC256_FIXTURE_COUNT][1 << PARAM_M] = {{0}};
    uint16_t rs_z_ref[HQC256_FIXTURE_COUNT][PARAM_N1] = {{0}};
    uint16_t rs_error_values_ref[HQC256_FIXTURE_COUNT][PARAM_N1] = {{0}};
    uint8_t rs_corrected_ref[HQC256_FIXTURE_COUNT][PARAM_N1] = {{0}};
    uint16_t rs_degree_ref[HQC256_FIXTURE_COUNT] = {0};
    ...
    for (size_t fixture = 0; fixture < HQC256_FIXTURE_COUNT; ++fixture) {
        memcpy(codeword_words[fixture], hqc256_fixture_codewords[fixture], VEC_N1N2_SIZE_BYTES);
        reed_muller_decode(rs_words[fixture], codeword_words[fixture]);
        reed_solomon_decode(message_words, rs_words[fixture]);
        ok &= memcmp(message_words, hqc256_fixture_expected_messages[fixture], PARAM_K) == 0;
        ...
        hqc_rs_bench_compute_roots(rs_error_ref[fixture], rs_sigma_ref[fixture]);
        hqc_rs_bench_compute_z_poly(rs_z_ref[fixture], rs_sigma_ref[fixture], rs_degree_ref[fixture], rs_syndromes_ref[fixture]);
        hqc_rs_bench_compute_error_values(rs_error_values_ref[fixture], rs_z_ref[fixture], rs_error_ref[fixture]);
        ...
        ok &= memcmp(rs_corrected_ref[fixture] + (PARAM_G - 1), hqc256_fixture_expected_messages[fixture], PARAM_K) == 0;
    }

    for (int iter = 0; iter < HQC256_BENCH_ITERS; ++iter) {
        for (size_t fixture = 0; fixture < HQC256_FIXTURE_COUNT; ++fixture) {
            switch (HQC256_SUBSTAGE) {
                case 1: {
                    rm_codeword_t *code_array = (rm_codeword_t *)codeword_words[fixture];
                ...
                case 6: {
                    ...
                    hqc_rs_bench_compute_roots(error, sigma);
                ...
                case 8: {
                    ...
                    hqc_rs_bench_compute_error_values(error_values, rs_z_ref[fixture], rs_error_ref[fixture]);

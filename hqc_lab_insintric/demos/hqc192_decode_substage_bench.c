...
#define HQC192_BENCH_ITERS 10
#endif

#ifndef HQC192_SUBSTAGE
#define HQC192_SUBSTAGE 0
#endif
...
int main(void) {
    uint64_t codeword_words[HQC192_FIXTURE_COUNT][VEC_N1N2_SIZE_64] __attribute__((aligned(128))) = {{0}};
    uint64_t rs_words[HQC192_FIXTURE_COUNT][VEC_N1_SIZE_64] __attribute__((aligned(128))) = {{0}};
    uint64_t message_words[MSG_WORDS] = {0};

    rm_expanded_cdw rm_expanded_ref[HQC192_FIXTURE_COUNT][PARAM_N1] __attribute__((aligned(128)));
    rm_expanded_cdw rm_transform_ref[HQC192_FIXTURE_COUNT][PARAM_N1] __attribute__((aligned(128)));
    ...
    uint8_t rs_corrected_ref[HQC192_FIXTURE_COUNT][PARAM_N1] = {{0}};
    uint16_t rs_degree_ref[HQC192_FIXTURE_COUNT] = {0};

    int ok = 1;
    unsigned total_ops = 0;
    volatile uint32_t checksum = 0;

    for (size_t fixture = 0; fixture < HQC192_FIXTURE_COUNT; ++fixture) {
        memcpy(codeword_words[fixture], hqc192_fixture_codewords[fixture], VEC_N1N2_SIZE_BYTES);
        reed_muller_decode(rs_words[fixture], codeword_words[fixture]);
        reed_solomon_decode(message_words, rs_words[fixture]);
        ok &= memcmp(message_words, hqc192_fixture_expected_messages[fixture], PARAM_K) == 0;
        ...
        ok &= memcmp(rs_corrected_ref[fixture] + (PARAM_G - 1), hqc192_fixture_expected_messages[fixture], PARAM_K) == 0;
    }

    for (int iter = 0; iter < HQC192_BENCH_ITERS; ++iter) {
        for (size_t fixture = 0; fixture < HQC192_FIXTURE_COUNT; ++fixture) {
            switch (HQC192_SUBSTAGE) {
                case 1: {
                    rm_codeword_t *code_array = (rm_codeword_t *)codeword_words[fixture];

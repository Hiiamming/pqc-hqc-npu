...
EOF_WORKLOAD
    ...
    set +e
    "$ADB" shell "cd '$ENERGY_REMOTE_DIR' && SAMPLE_INTERVAL='$DIRECT_SAMPLE_INTERVAL' IDLE_SECONDS='$DIRECT_IDLE_SECONDS' IDLE_POSITION='$DIRECT_IDLE_POSITION' ENERGY_TMP_DIR='$ENERGY_REMOTE_DIR' ./measure_board_energy.sh '$label' sh '$remote_workload' > '${remote_out}.log' 2>&1"
    local rc=$?
    set -e

...
HMX_BATCH="${HQC_RM_HMX_BATCH:-1}"
OUT="$PROJECT_DIR/build/hqc1_decode_stage_bench_hexagon_stage${STAGE}"
mkdir -p "$(dirname "$OUT")"

echo "=== Compiling HQC-128 stage benchmark for HMX-32/HVX-tail path, hmx_batch=$HMX_BATCH, stage=$STAGE, iters=$BENCH_ITERS ==="
"$CLANG" -O2 -mv75 \
    -ffunction-sections -fdata-sections \
    -mhvx -mhvx-length=128B \
    -mhmx \
    -DARCHV=75 \
    -DHQC_RM_HMX_BATCH="$HMX_BATCH" \
    -DHQC1_STAGE="$STAGE" \
    ...
    -Wl,--gc-sections \
    -o "$OUT" \
    "$PROJECT_DIR/demos/hqc1_decode_stage_bench.c" \

...
#define HQC3_BENCH_ITERS 10
#endif

#ifndef HQC3_SUBSTAGE
#define HQC3_SUBSTAGE 0
#endif
...
int main(void) {
    uint64_t codeword_words[HQC3_FIXTURE_COUNT][VEC_N1N2_SIZE_64] __attribute__((aligned(128))) = {{0}};
    uint64_t rs_words[HQC3_FIXTURE_COUNT][VEC_N1_SIZE_64] __attribute__((aligned(128))) = {{0}};
    uint64_t message_words[MSG_WORDS] = {0};

    rm_expanded_cdw rm_expanded_ref[HQC3_FIXTURE_COUNT][PARAM_N1] __attribute__((aligned(128)));
    rm_expanded_cdw rm_transform_ref[HQC3_FIXTURE_COUNT][PARAM_N1] __attribute__((aligned(128)));

    uint8_t rs_cdw_ref[HQC3_FIXTURE_COUNT][PARAM_N1] = {{0}};
    uint16_t rs_syndromes_ref[HQC3_FIXTURE_COUNT][2 * PARAM_DELTA] = {{0}};
    uint16_t rs_sigma_ref[HQC3_FIXTURE_COUNT][1 << PARAM_SIGMA_SIZE_LOG] = {{0}};
    uint8_t rs_error_ref[HQC3_FIXTURE_COUNT][1 << PARAM_M] = {{0}};
    uint16_t rs_z_ref[HQC3_FIXTURE_COUNT][PARAM_N1] = {{0}};
    uint16_t rs_error_values_ref[HQC3_FIXTURE_COUNT][PARAM_N1] = {{0}};
    uint8_t rs_corrected_ref[HQC3_FIXTURE_COUNT][PARAM_N1] = {{0}};
    uint16_t rs_degree_ref[HQC3_FIXTURE_COUNT] = {0};
    ...
    for (size_t fixture = 0; fixture < HQC3_FIXTURE_COUNT; ++fixture) {
        memcpy(codeword_words[fixture], hqc3_fixture_codewords[fixture], VEC_N1N2_SIZE_BYTES);
        reed_muller_decode(rs_words[fixture], codeword_words[fixture]);
        reed_solomon_decode(message_words, rs_words[fixture]);
        ok &= memcmp(message_words, hqc3_fixture_expected_messages[fixture], PARAM_K) == 0;
        ...
        ok &= memcmp(rs_corrected_ref[fixture] + (PARAM_G - 1), hqc3_fixture_expected_messages[fixture], PARAM_K) == 0;
    }

    for (int iter = 0; iter < HQC3_BENCH_ITERS; ++iter) {
        for (size_t fixture = 0; fixture < HQC3_FIXTURE_COUNT; ++fixture) {
            switch (HQC3_SUBSTAGE) {
                case 1: {
                    rm_codeword_t *code_array = (rm_codeword_t *)codeword_words[fixture];

...
static void hqc192_decode_direct(uint8_t message[PARAM_K], const uint64_t codeword[VEC_N1N2_SIZE_64])
{
...
int main(void)
{
    ...
    for (int i = 0; i < HQC192_BENCH_ITERS; ++i) {
        for (size_t fixture = 0; fixture < HQC192_FIXTURE_COUNT; ++fixture) {
            memcpy(codeword_words, hqc192_fixture_codewords[fixture], VEC_N1N2_SIZE_BYTES);
            hqc192_decode_direct(recovered, codeword_words);
            ok &= memcmp(hqc192_fixture_expected_messages[fixture], recovered, sizeof(recovered)) == 0;
            total_rs_errors += hqc192_fixture_rs_symbol_errors[fixture];
            ++total_decodes;
        }
    }

...
int main(void)
{
    ...
    uint64_t rs_references[HQC3_FIXTURE_COUNT][VEC_N1_SIZE_64] __attribute__((aligned(128))) = {{0}};
    uint64_t rs_words[VEC_N1_SIZE_64] = {0};
    uint64_t message_words[MSG_WORDS] = {0};
    int ok = 1;
    ...
    for (size_t fixture = 0; fixture < HQC3_FIXTURE_COUNT; ++fixture) {
        ...
        if (memcmp(hqc3_fixture_expected_messages[fixture], recovered, sizeof(recovered)) != 0) {

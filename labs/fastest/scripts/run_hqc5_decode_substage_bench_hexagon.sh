...
if [ ! -f "$PROJECT_DIR/fixtures/hqc5_decode_fixture.c" ]; then
    "$SCRIPT_DIR/gen_hqc5_decode_fixture.sh"
fi

SUBSTAGE="${HQC5_SUBSTAGE:-4}"
BENCH_ITERS="${HQC5_BENCH_ITERS:-10}"
OUT="$PROJECT_DIR/build/hqc5_decode_substage_bench_hexagon_stage${SUBSTAGE}"
mkdir -p "$(dirname "$OUT")"

echo "=== Compiling HQC-256 substage benchmark for Hexagon fastest HVX/HMX path, substage=$SUBSTAGE, iters=$BENCH_ITERS ==="
"$CLANG" -O2 -mv75 \
    ...
    -DARCHV=75 \
    -DHQC5_SUBSTAGE="$SUBSTAGE" \
    -DHQC5_BENCH_ITERS="$BENCH_ITERS" \
    ...
    -Wl,--gc-sections \
    -o "$OUT" \
    "$PROJECT_DIR/demos/hqc5_decode_substage_bench.c" \
    "$PROJECT_DIR/fixtures/hqc5_decode_fixture.c" \

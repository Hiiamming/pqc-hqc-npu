...
SUBSTAGE="${HQC3_SUBSTAGE:-4}"
BENCH_ITERS="${HQC3_BENCH_ITERS:-10}"
OUT="$PROJECT_DIR/build/hqc3_decode_substage_bench_hexagon_stage${SUBSTAGE}"
mkdir -p "$(dirname "$OUT")"

echo "=== Compiling HQC-192 substage benchmark for Hexagon fastest HVX/HMX path, substage=$SUBSTAGE, iters=$BENCH_ITERS ==="
"$CLANG" -O2 -mv75 \
    ...
    -Wl,--gc-sections \
    -o "$OUT" \
    "$PROJECT_DIR/demos/hqc3_decode_substage_bench.c" \

...
int main(void)
{
    ...
    uint64_t rs_references[HQC128_FIXTURE_COUNT][VEC_N1_SIZE_64] __attribute__((aligned(128))) = {{0}};
    uint64_t rs_words[VEC_N1_SIZE_64] = {0};
    uint64_t message_words[MSG_WORDS] = {0};
    int ok = 1;
    ...
    for (size_t fixture = 0; fixture < HQC128_FIXTURE_COUNT; ++fixture) {
        ...
        if (memcmp(hqc128_fixture_expected_messages[fixture], recovered, sizeof(recovered)) != 0) {
            printf("[stage-bench] setup fixture=%zu result=FAIL\n", fixture);
            return 1;
        }
    }

    for (int i = 0; i < HQC128_BENCH_ITERS; ++i) {
        for (size_t fixture = 0; fixture < HQC128_FIXTURE_COUNT; ++fixture) {
#if HQC128_STAGE == 1
            memcpy(codeword_words, hqc128_fixture_codewords[fixture], VEC_N1N2_SIZE_BYTES);

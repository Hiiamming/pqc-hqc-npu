...
The paper suite builds each level, deploys the ARM64 CPU scalar executable and
the FastRPC cDSP fastest artifacts, then measures direct energy and process CPU.
It writes the final comparison table to:

```text
README_result_whole.md
```

The paper protocol uses `5` repeats and `32000` decodes per backend and HQC
level. qprof is disabled during energy collection because it can perturb power
and clock state. Run qprof separately when diagnostic context is needed.

For a one-repeat regression check:

```sh
scripts/measure_android.sh --suite paper --levels "128" --repeats 1
```
